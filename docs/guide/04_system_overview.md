...
データが流れる道筋。この章で「どこで何が・なぜそうなっているか」の地図ができ、
...
グラフ部品は形式ごとの専用コンポーネントに分かれていく。新しい形式（例: 保険）を
足すたびに「DBにカラム追加 → 取込に分岐追加 → APIに項目追加 → フロントに部品追加 →
Chrome拡張にも複製」と**全層を触る**ことになる。実際、旧実装ではIFRS企業の連結が
0円で保存されて表示できない、という形でこの問題が起きていた。
