...
| 取り決め2: チャート構造 | ③ → ④ | 積み上げはバーとセグメント、ウォーターフォールはステップの構造をそのまま返す。セグメントは固定キーなしの配列 | 型をGraphQLスキーマで固定。形式が増えても構造は変わらない（中身は[04章](04_backend.md)） |
...
| 層 | 新形式（例: 保険）追加時の作業 |
|---|---|
| ① 取込 | Extractorクラスを1つ追加 + 判定表に1行 + 形式レジストリに定数と対応を追加 |
| ② 保存 | **なし**（マイグレーション不要） |
| ③ 表示 | Builderクラスを1〜2つ追加 + 登録表（BS・PL）に1行ずつ |
| ④ フロント / Chrome拡張 | **なし** |
...
この章の全体を1つの科目の実データで追ったデータフロー図は、[07章](07_taxonomy_mapping.md)の冒頭に置いている。
