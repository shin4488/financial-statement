...
タグ名だけでなく、財務諸表の構造そのものも変わる。この「会計基準 × 開示様式」の組合せを、このシステムでは**形式**と呼ぶ（[02章](02_product.md)で見た6種類）。
...
|---|---|---|---|
| 取り決め1: 科目コード | ① → ② → ③ | `bs.assets` など42種の共通語彙。命名は `<財務諸表>.<英名スネークケース>` | 定義をRubyの定数1ファイルに限定。「Extractorが使うコード ⊆ 定義」をspecで機械検証 |
...
実際に保険（`jgaap_insurance`）を追加したときも触ったのはこの範囲だけだった。骨格が一般と同じ業種（建設・鉄道など）は形式すら増やさず、Extractorのフォールバックリストへの追記で済む。
