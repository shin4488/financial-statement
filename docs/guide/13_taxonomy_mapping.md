# 13. XBRLタグ ↔ 科目コード ↔ Builder 対応表

「このタグはどの科目コードになり、どのグラフで使われるか」を引くための表。XBRLタグを扱う作業のときに開く。

- タグの実測値・企業ごとの差異は [14_xbrl_research.md](14_xbrl_research.md)
- なぜこの3段構えなのかは [09_layering.md](09_layering.md)
...
取れないことをエラーではなく正常系として扱い、チャート単位で表示を落とす設計にしてある（カード全体は消さない）。詳細は [12_serving.md](12_serving.md)。

---

## 新しいタグを追加するとき

1. 対象企業の有報XBRLを取得して実際のタグを確認する（[14](14_xbrl_research.md) の手順）
