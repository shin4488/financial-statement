...
[01章](01_financial_knowledge.md)で見た財務諸表のデータが、EDINETのXBRLからどう取り込まれ、DBに保存され、GraphQLで配信され、チャートとして描かれるか。この章はその全行程を、データの流れる順に1本で追う。設計の「なぜ」は、それが効いている場所に織り込んで説明する。
...
会計基準や業種によって、原本のタグ名やグラフの内訳は異なる。この違いを取込時とチャート作成時に処理し、保存形式とフロントエンドを共通にする。
...
実際の取込ではID名だけで当期を決めない。`Xbrl::Document` はXMLのfactとcontextを索引化し、`Xbrl::Context` は企業・日付・ディメンションを解析する。`Xbrl::ReportingPeriod` が連結／単体の対象期間を決めて、同じ意味のcontextから科目を選ぶ。IDが任意の名称でも同じ結果になり、標準名でも別年度や別企業の数値は使わない。連結・単体の期間差や重複候補の扱いは[06章の期首・期末残高](06_taxonomy_mapping.md#roeroaの期首期末残高)を参照。
...
変換先となる、全形式共通のコード。命名は `<財務諸表>.<英名スネークケース>`（`bs.assets`・`pl.revenue` など。全コードは `FinancialStatements::ItemCodes::ALL` を参照）。
...
合計額を直接取得できない場合に限り、原本にある内訳を足し合わせる。数値の選び方は次のとおり。
...
| 開示のない数値を0としない | 対象の内訳のうち、開示されたものを足す。1つもなければ「データなし」とする | 事業の組合せは企業によって異なる。「開示がない」と「金額が0」は区別する |
...
`FinancialStatements::Indicators` は、チャートと同じ `FinancialStatement#items_hash` からROE・ROA・売上高純利益率・総資産回転率・財務レバレッジを計算する。チャートのセグメントや丸め済みの比率は使わない。保存形式・連結区分の解釈はバックエンドに留め、画面では計算しない。

- 利益は連結で `pl.profit_attributable_to_owners`、単体で `pl.profit` に統一する。連結の値が欠けても全体利益へ置き換えない。
- 総資産・自己資本は同じ有報の期首期末平均。自己資本は純資産合計と区別する。取得式は[06章](06_taxonomy_mapping.md#roeroaの期首期末残高)。
...
取込時にXBRLの`decimals`から1表示単位を求め、科目の`rounding_error`へ保存する。切捨て開示も考慮した上限で、`INF`は0、精度不明はNULLとする。合算科目は入力の上限を合計し、未知を含む場合は未知のまま保持する。候補選択では採用した金額に対応する精度を保持する。

この情報は将来の検算用で、現在の表示判定・許容差は変更しない。既存行はNULLのまま利用でき、新規取込・再取込で精度が付く。追加取得するPL・CF科目も現行のチャートでは未使用。
