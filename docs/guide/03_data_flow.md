...
変わらないもの、すなわち**科目コード**（`bs.assets` など全形式共通の42種）を中央に置き、変わるものを入口と出口に寄せる。
...
変換先となる、全形式共通のコード。命名は `<財務諸表>.<英名スネークケース>`（`bs.assets`・`pl.revenue` など42種）。
...
このほか旧実装の `security_reports` テーブルが凍結保管されている（コードからの参照はゼロ。扱いは[07章](07_unused_but_kept.md)）。
