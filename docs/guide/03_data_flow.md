...
データの中身ではなくシステム側の話（構成・取込ジョブの信頼性・画面の実装）は[04章](04_system.md)、個々のXBRLタグと科目コードの対応を引く表は[06章](06_taxonomy_mapping.md)。
...
| [保存](#保存-dbのデータモデル) | データモデルと「行がない = 開示なし」の規約 |
...
変わらないもの、すなわち**科目コード**（`bs.assets` など全形式共通）を中央に置き、変わるものを入口と出口に寄せる。
...
層の間で受け渡すデータの形は、科目コード（「取込」の節）とチャート構造（「チャート組み立て」の節）の2つで固定している。
...
この方針の帰結として、科目が拡張タグでしか開示されていない企業ではその科目が取れない。NTTの収益は経営指標サマリ（`jpcrp_cor`）の標準タグへのフォールバックで救済でき、東京海上HDはサマリにも無いためPLだけ表示不可になる（表示不可を正常系として扱う仕組みは「チャート組み立て」の節。実測の記録は[06章](06_taxonomy_mapping.md)）。
...
- IFRSの2様式はDEI（書類メタ情報）では区別できず、タグの実在で判定する（楽天のfactダンプで確認。根拠は[06章](06_taxonomy_mapping.md)）
...
変換先となる、全形式共通のコード。命名は `<財務諸表>.<英名スネークケース>`（`bs.assets`・`pl.revenue` など）。
...
| `consistent("…:A", sum("…:B", "…:C"), components: [...])` | 候補同士、または指定した構成科目との一致で総額を確認 | 営業収益と売上高+営業収入の候補を、営業原価+営業総利益で検算 |

候補の大小では総額を決めない。構成科目が揃えばその合計と照合し、揃わなければ候補同士の一致を要求する。どちらでも確認できない候補は保存しない。イオンの実XBRLでは営業原価6,804,966＋営業総利益3,910,376＝営業収益10,715,342百万円で確認できる。売上高9,355,439百万円を単純に最大値比較から除外する方式ではない。
...
普通の科目は合計のタグを「引く」だけで済むが、鉄道事業会計規則のように損益を事業区分ごとに並べる様式では、合計が1つのタグとして開示されていない。引けないものは内訳を「足す」しかない、というのが `sum(...)` の役割。
...
| 決めごと | 内容 | 理由 |
|---|---|---|
| 足すのは「合計タグを引けない開示」だけ | 鉄道単体・電気通信・海運の営業収益/費用（合計タグ自体が無い）、IFRSののれん＋無形（別掲企業がある）、売上高+営業収入（総額タグを付けない企業がある。上記`consistent`の候補） | 合計タグが引けるならそれが一次情報 |
...
| 逆算はしない | 「営業費＋営業利益」で求めない | 開示値の等式で検算し、未取得科目を逆算して開示科目として扱わない |
...
同じ抽出処理から、形式によって違う科目が出てくる（キーが無い = 開示なし）。
...
全科目のタグ対応表と根拠の実測記録は[06章](06_taxonomy_mapping.md)（**XBRLタグを触る作業の前に該当する表は必読**）。
...
ここから参照系。検索で有報を絞り（検索の仕様は「GraphQL API」の節）、形式ごとのBuilderが科目コードの行からチャートの構造そのものを組み立てる。
...
```
WaterfallChart（CF用。5必須段に、開示された調整科目を追加）
├ renderable: 描けるか
├ note: renderable=false のとき表示する説明文
└ steps: [
...
| 構造の仕掛け | 吸収する業務上の差異 |
|---|---|
| `segments[]` をそのまま積む（固定キーなし） | 形式ごとの段数・科目の違い（銀行BS 資産合計1段 / IFRS分類BS 流動・非流動2段…） |
...
| `kind: balance / flow` | CFの残高（0起点）と増減（浮かせる）の描き分け。為替換算差額や連結範囲変更は開示タグの増減として追加 |

### 実例1: IFRSのPLを開示科目で検算する

IFRSのPLは収益、原価・販管費（または営業費用合計）、その他収益・費用、金融収益・費用、持分法損益から税引前利益までを検算する。原価・販管費と営業費用合計を同時には積まない。研究開発費が別掲される構成も試すが、注記の研究開発費が既に他の費用に含まれる場合を避けるため、全体の等式が成立する構成だけを使う。

NTT・楽天の実XBRLでは開示科目から税引前利益まで照合できる。武田薬品には企業拡張タグの製品関連無形資産償却・減損があり、現在の標準タグ抽出だけでは網羅できない。この場合は差額で「その他損益」を作らず、当該PLを理由付きで表示対象外にする。詳細のないIFRSサマリも、収益と税引前利益だけでは費用を確定できないためPLを表示しない。

銀行・保険・IFRS流動性配列のBSでは、開示された資産合計・負債合計・純資産（資本）を使う。「その他資産」「その他負債」を差引きで生成しない。

### 実例2: 表示不可も正常系（東京海上のPL）

保険IFRSは収益が企業拡張タグのみで標準タグから取れない（「XBRLとタクソノミの読み方」の節）。その場合も**カード全体を落とさず、取れなかったチャートだけ**説明文にする。

```json
{ "profitLoss":   { "renderable": false,
                    "note": "損益計算書: 開示された収益・費用・税引前利益の関係を確認できないため表示に対応していません。" },
...
`jgaap_general` のPLは業種によって費用科目の組合せが違う。Builderは次の構成を順に試し、**借方合計（費用+営業利益）が売上と開示精度の範囲で合う最初の構成**で描く（開示された科目だけを積む）。

| 順 | 構成 | 当てはまる業種 | この順に置く理由 |
|---|---|---|---|
| 1 | 売上原価・金融費用・販管費、ガスの雑営業・附帯事業費用（あるものだけ） | 一般事業会社（原価+販管費）、証券（金融費用+販管費）、原価と販管費の内訳を営業費用と併記する鉄道連結など | 内訳と一括を併記する企業で、情報量が多い内訳を捨てないため先頭 |
...
| 形式 | BS | PL | CF |
|---|---|---|---|
| `jgaap_general` | 流動/固定の4段（3分類を持たない業種は固定資産1段） | 費用（構成は業種で異なる。実例3）+営業利益 | 全形式共通（期首残→営業→投資→財務→開示された換算差額・連結範囲変更→期末残） |
| `jgaap_bank` | 資産合計／負債合計＋純資産 | 経常収益/費用/利益（`PlJgaapFinancialInstitution`。保険と共通） | 同上 |
| `jgaap_insurance` | 資産合計／負債合計＋純資産 | 銀行と共通 | 同上 |
| `ifrs_classified` | 流動/非流動 | 開示収益・費用・持分法損益と税引前利益を検算 | 同上 |
| `ifrs_liquidity` | 資産合計／負債合計＋資本 | ifrs_classifiedと共通 | 同上 |
| `ifrs_summary` | 「詳細データが収録されていない」（renderable: false。負債の実値がサマリに無く残差でも描かない） | 費用を確定できないため表示不可 | 同上 |
...
- **総額・内訳・利益・残高変動を開示値の等式で検算する**。一律10%の許容差は設けない。
- XBRLの`decimals`から表示単位を求め、比較に使う科目の丸め誤差上限の和より小さい差だけを許容する。切捨て開示も考慮して1表示単位未満を上限とする。`INF`は誤差なし、精度不明は完全一致のみ。合算科目は各入力の誤差を合算し、選択科目は採用した入力の精度を引き継ぐ。
- 誤差上限は`financial_statement_items.rounding_error`に保存し、`FinancialStatements::Amounts`でBuilderへ渡す。既存行の精度は推測せずNULLとする。丸めのある既存データは再取込まで表示対象外になり得る。
- BSは資産合計＝負債合計＋純資産を確認し、表示比率の分母を常に資産合計にする。内訳が合計を説明できなければ合計表示に戻す。繰延資産などの欠落を小さい分母で隠さない。
- CFは5必須科目に加え、取得できた為替・連結範囲変更を使って期末残高を検算する。説明できない差を期末残高へ飛ばして隠さない。
- 債務超過は3本目のバー「債務超過」を作り、透明な`spacer`で高さを合わせて描く
- 比率はBigDecimalで計算して小数1桁に切り捨て（「19.900000000000002%」や合計100%超を防ぐ）

検算方針の参考: [金融庁・計算リンクによる親子科目の整合性確認](https://www.fsa.go.jp/search/20081021.html)、[XBRLの丸め精度と計算要件](https://www.xbrl.org/REQ/calculation-requirements-1.1/REQ-2022-05-25/calculation-requirements-1.1-2022-05-25.html)、[IAS 7の現金残高照合](https://www.ifrs.org/issued-standards/list-of-standards/ias-7-statement-of-cash-flows.html/)。本実装は保存した科目の等式を検算するもので、XBRL計算リンク全体の検証器ではない。
