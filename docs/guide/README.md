...
| 部 | 章 | 役割 |
|---|---|---|
| 学ぶ章 | 01〜08 | 前提知識ゼロから全体を理解する。**番号順に読む** |
| 深掘り章 | 09〜12 | 設計判断の「なぜ」と詳細。設計変更や込み入った修正の前に読む |
| 資料 | 13〜15 | 作業のときに引く一覧表・実測データ・記録 |
...
| 07 | [開発の進め方](07_development.md) | 手順書の在り処・スキーマ変更の連鎖手順・submodule構成のPR運用 | 開発に参加する人 |
| 08 | [本番運用](08_operations.md) | 本番環境の構成・デプロイ・日次バッチの監視とリカバリ | 運用に関わる人 |

## 深掘り章（09〜12）

学ぶ章が「どう動くか」を説明するのに対し、深掘り章は「なぜそう作ったか」の判断根拠と、
学ぶ章に収まらない詳細・実データでの実例を記録する。09→10→11→12の順で
設計の考え方 → データの形 → 処理の流れ、の順に頭に入る。

| # | 文書 | 内容 |
|---|---|---|
| 09 | [設計思想](09_layering.md) | **深掘りの最初に読む。** 会計基準ごとの差異をどこに閉じ込めるか。層の分け方と、どこに何を書くべきかの判断基準 |
| 10 | [データモデル詳細](10_data_model.md) | 実データ例・企業名を2箇所に持つ理由・「行がない = 開示なし」の規約 |
| 11 | [取込層の詳細](11_ingestion.md) | DEIを検証する理由・形式判定の実装注意・新形式の追加手順 |
| 12 | [表示層の詳細](12_serving.md) | チャート契約の仕様・実データでの実例3つ・GraphQLの設計理由と実測値 |

## 資料（13〜15）

| # | 文書 | 使うとき |
|---|---|---|
| 13 | [タグ対応表](13_taxonomy_mapping.md) | XBRLタグを扱う作業のとき。タグ ↔ 科目コード ↔ Builder の一覧 |
| 14 | [XBRL実地調査](14_xbrl_research.md) | 新しい形式に対応するとき。6社・4形式の実測データ。**XBRLタグを触る前に必読** |
| 15 | [旧系統の削除記録](15_legacy_cleanup.md) | 凍結データ（`security_reports`）の扱いを判断するとき・削除の経緯を確認するとき |
