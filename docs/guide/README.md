...
| 03 | [システム全体像と設計](03_system_overview.md) | 構成（リポジトリ・Docker）・このアプリ固有の難しさと解き方（4層設計）・データモデル・データの流れ。SPA・コンテナなど技術の基礎説明つき | 全員 |
...
| 06 | [開発と運用](06_development_operations.md) | ブランチ・PR運用・本番構成・デプロイ・監視とリカバリ | 開発・運用に関わる人 |
| 07 | [XBRLタグ対応表と実地調査](07_taxonomy_mapping.md) | 【資料】1科目の実データで追うデータフロー図、名前空間・コンテキストの読み方、タグ ↔ 科目コード ↔ Builderの一覧、根拠になった8社の実測記録。**XBRLタグを触る前に対応表（「実地調査の記録」より前）は必読** | タグを扱う作業のとき |
