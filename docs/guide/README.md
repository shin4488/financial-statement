...
flowchart LR
    ...
    subgraph S["しくみを知る"]
        D["04<br>システム全体像"] --> E["05<br>バックエンド実装"]
        D --> F["06<br>フロントエンド実装"]
    end
    subgraph U["動かす"]
        G["07<br>開発と運用"]
...
| # | 文書 | 使うとき |
|---|---|---|
| 12 | [XBRLタグ対応表と実地調査](12_taxonomy_mapping.md) | XBRLタグを扱う作業のとき（**必読**）。タグ ↔ 科目コード ↔ Builder の一覧、科目名のゆれを吸収するフォールバック順、6社・4形式の実測データ |
| 13 | [旧系統の削除記録](13_legacy_cleanup.md) | 凍結データ（`security_reports`）の扱いを判断するとき・削除の経緯を確認するとき |
