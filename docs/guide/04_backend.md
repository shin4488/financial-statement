...
#### なぜ合算記法があるのか（そして差し引きはしないのか）

Extractorは「開示された金額をそのまま保存する」のが規約で、単一タグもフォールバックも**辞書からタグの値を引くだけ**で計算はしない。ところが鉄道事業会計規則の様式は、損益を事業区分ごとに並べる形で、**営業収益の合計が1つのタグとして開示されていない**（レシートに商品ごとの金額はあるのに合計行がない状態）。

```
鉄道事業  営業収益 2,020,442   営業費 1,795,414   営業利益 225,028
関連事業  営業収益   205,293   営業費   128,314   営業利益  76,978
                                          全事業営業利益 302,007   ← 合計はここにしかない
```

合計を「引く」ことはできないので、内訳を「足す」しかない。それを表すのが `sum(...)` で、使っているのは合計タグの無い科目だけ: 鉄道単体（営業収益・営業費）、電気通信（電気通信事業＋附帯事業）、海運単体（海運業＋その他事業）、IFRSののれん＋無形資産（合算タグを出す会社と別掲する会社がある）の4か所。

- **足してよい根拠**: 法令様式が「区分の和＝合計」と定義している項目だけを足す。JR東日本なら 2,225,735 は営業費の合計 1,923,728 と全事業営業利益 302,007 の和に一致し、推計ではなく開示値どおりの合計になる
- **存在するタグだけ足す**: 区分の組合せは会社ごとに違う（鉄道事業のみ／鉄道＋不動産＋開発／…）ため、1つのリストで全パターンを引けるようにしている。1つも無ければ「開示なし」（0を保存しない）
- **フォールバックの最後に置く**: 合計タグを開示している会社（東急など）ではそちらが一次情報。合計があればそれ、無いときだけ足す
- **差し引きで求めない**: 「営業収益＝営業費＋営業利益」と逆算すれば書けるが、それは開示値ではなく推計になる。取込では開示された数字（とその単純な合計）だけを保存し、差し引きで埋める処理は表示側のBuilderで「その他損益（純額）」のように導出項目だと分かる形に限定する。合算と逆算を混ぜないことで、DBの値が「開示値そのもの」であることを保つ
