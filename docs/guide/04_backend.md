...
fact → 辞書 → 科目コードと変換される全体像は、[07章](07_taxonomy_mapping.md)冒頭のデータフロー図のとおり。Extractorは実質「タグ → 科目コードの対応表」で、ロジックをほぼ持たない。対応表の値は4記法で、**企業・業種ごとの科目名のゆれをここで吸収する**。
...
`max` が要るのは、制度上は「営業収益 = 売上高 + 営業収入」なのに、企業によって (a) 営業収益を総額として3タグとも付ける小売、(b) 総額タグを付けず売上高と営業収入だけ付ける小売、(c) 売上高を総額とし営業収益を一部の事業にだけ付ける会社、(d) 営業収入だけを開示する持株会社の単体、が混在するため。「先に見つかった方」では (b)(c) で総額を取り損ねる（3年分の実測で実際に見つかった。[07章](07_taxonomy_mapping.md)）が、「大きい方」ならどのパターンでも総額になる。
...
| 決めごと | 内容 | 理由 |
|---|---|---|
| 足すのは合計タグの無い科目だけ | 鉄道単体・電気通信・海運単体の営業収益/費用、IFRSののれん＋無形 の4か所 | それ以外は合計タグを引ける |
