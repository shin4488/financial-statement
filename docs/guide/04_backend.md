...
fact → 辞書 → 科目コードと変換される全体像は、[07章](07_taxonomy_mapping.md)冒頭のデータフロー図のとおり。Extractorは実質「タグ → 科目コードの対応表」で、ロジックをほぼ持たない。1科目に複数タグを並べると先頭から順に探すフォールバックになり、**企業・業種ごとの科目名のゆれをここで吸収する**。
