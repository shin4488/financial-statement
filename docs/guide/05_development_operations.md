...
`20260910000001`のマイグレーションで科目に`rounding_error`を追加するため、backend更新時に`RAILS_ENV=production bundle exec rails db:migrate`を実行する。既存行はNULLのまま保持し、表示単位を推測して埋めない。既存有報の丸め精度・追加損益・CF調整科目を利用するには通常の`ingestion:documents`または期間指定の`ingestion:backfill`で逐次再取込する。再取込前は完全一致する表のみ表示でき、再取込後も未対応の拡張タグが必要な表は説明文になる。本番反映・再取込は別途実施する。
