...
EDINETはHTTP 200の本文に `metadata.status: 404` 等を返すことがある。`Edinet::Client::ApiError#status` で取得不能と認証・制限エラーを区別する。404の書類をZIP破損として繰り返し取得せず、403/429では連続再試行をしない。財務数値のない訂正書類は既存の科目・公表ROEを保持するため、確認日時が未設定でも再試行で解消しない場合がある。
