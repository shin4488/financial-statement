...
このタスクは対応済み形式の主要財務諸表について、期首総資産・期首自己資本・期末自己資本のいずれかが不足する有報全体を再取得して科目を入れ替える。実行前にDBを退避し、既存の日次取込と同じ逐次・間隔付きの実行を使う。EDINETリクエストを並列化しない。元資料に期首がない有報は再実行しても対象に残るため、日次ジョブには追加しない。失敗したdocIDの再実行には `ingestion:documents` を使う。期首が開示されていない有報は期末だけで代用しない。

企業公表ROEの保存追加時は、マイグレーション後に `bundle exec rake ingestion:reingest_disclosed_roe` も実行する。計算可能な企業・単体・未対応形式を含め、抽出未確認の財務諸表を持つ有報が対象。通常取込で確認日時を記録するため、未開示の有報も再実行で対象から外れる。途中失敗は未確認のまま残り再開可能。公表ROEでの補完は期首残高の代用ではなく、別定義の企業公表値を表示するもの。

証券コードが空欄の過年度書類も、保存済みの書類ID・EDINET企業コード・会計期間が一致する場合に限って再取込できる。企業名や年度の推測で新規に紐付けない。ファンド書類は常に対象外とし、過去に誤登録した書類も科目を保持したまま `is_primary: false` にして企業比較から外す。

EDINETはHTTP 200の本文にエラーを返すことがある。公式仕様の400/404/500は `metadata.status`、401/429は直下の `StatusCode` にあり、どちらも確認する。`Edinet::Client::ApiError#status` で取得不能と認証・制限エラーを区別する。404の書類をZIP破損として繰り返し取得せず、403/429では連続再試行をしない。財務数値のない訂正書類は既存の科目・公表ROEを保持するため、確認日時が未設定でも再試行で解消しない場合がある。

検証は新規取込したテストデータだけで完了としない。閲覧環境の公開APIを全ページ取得して移行前後の指標の状態を集計し、残った欠損を元資料未開示・未対応形式・取得失敗などに分ける。ブラウザを再読込し、既存データが並ぶ実際の一覧でも表示を確認する。
