...
「売上（収益）− 費用 = 利益」の構造。売上を「何に使ったか（費用）+ 残った儲け（利益）」に
分解して左右に並べると、BSと同じく**左右の合計は必ず一致する**。
これがPLも積み上げ棒グラフ2本で描ける理由になる。
...
利益にはいくつか段階があり、このシステムでは本業の儲けを表す**営業利益**
（銀行は経常利益、IFRSは税引前利益）までを扱う。売上より費用が大きければ
**営業損失**（赤字）になり、そのときは損失が売上側（貸方）に積まれて高さが揃う。
...
期首の現金残高に3区分の増減を順に足していくと期末残高になる。これを階段状に描いたものが
ウォーターフォールグラフで、[02章](02_product.md)で見るCFチャートそのもの
（数字は説明用の例。█ = 残高・増加、▒ = 減少、1マス = 10）:
...
増減のバーが「直前の残高の高さから」浮いて始まるのが特徴で、現金がどこで増えて
どこで減ったかを一目で追える。なお実際には為替換算差額があるため
「期首 + 3区分 = 期末」は厳密には一致しない。この点はチャート実装にも影響する。
...
ため、このシステムは意図的に読まない（詳細は[08章](08_taxonomy_mapping.md)）。
