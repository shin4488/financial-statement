...
| IFRS・分類様式 | IFRS採用企業の連結（一般事業会社に多い） | −（IFRSの様式は業種コードでなくBSの中身で判定する） | 流動/非流動に区分（下図） | 売上収益 − 費用 → 税引前利益（下図。営業利益の開示は任意で企業差が大きい） |
...
このシステムは、日本基準で骨格そのものが違う銀行・保険を専用の形式で、それ以外の業種様式は一般事業会社と同じ形式で扱う（科目名の違いは取込側で吸収する。IFRSは業種様式によらず処理できる。詳細な対応状況は[Webガイド](https://investee.info/guide)）。
