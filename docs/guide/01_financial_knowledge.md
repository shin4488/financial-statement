...
同じ日本基準でも、銀行・保険など一部の業種は業法に基づく独自の様式で開示するため、業界によって財務諸表の形そのものが変わる（どの業種様式かは提出データの業種コードでわかる。後述のXBRL節で見るDEI）。またIFRSにもBSの様式が2種類ある。形の全体像は次のとおり。

| 財務諸表の形 | 対象 | BSの形 | PLの構造 |
|---|---|---|---|
| 日本基準・一般事業会社 | 製造・小売・ITなど大半（業種コードなし / `cte`） | 流動/固定に区分（BS節の図） | 売上高 − 費用 → 営業利益（PL節の図） |
...
| 日本基準・その他の業種様式 | 証券など、業法に基づく様式をもつ業種がほかにも複数ある | 業種ごとの固有様式 | 業種ごとに異なる |
| IFRS・分類様式 | IFRS採用企業の連結（一般事業会社に多い） | 流動/非流動に区分 | 売上収益 − 費用 → 税引前利益（営業利益の開示は任意で企業差が大きい） |
