...
このシステムが扱うデータそのものの話。会計やディスクロージャー制度になじみがない人向けに、「上場企業の財務情報が、どんな形式で、どこから手に入るのか」を順に説明する。
...
| 名前空間 | 内容 | タグの例 |
|---|---|---|
| `jpdei_cor` | DEI（書類情報）。会計基準・EDINETコード・連結の有無・会計期間など書類のメタ情報 | `AccountingStandardsDEI`（会計基準） |
...
- **IFRS採用企業でも、単体財務諸表は日本基準（`jppfs_cor`）でタグ付けされる**。IFRSが求められるのは連結のみのため
- IFRSのBS様式2種（下表）はDEIでは区別できず、タグの実在で判定する（判定の仕組みは[04章](04_system_overview.md)）

### 会計基準 × 業種で決まる財務諸表の形

同じ日本基準でも、銀行・保険など一部の業種は業法に基づく独自の様式で開示するため、業界によって財務諸表の形そのものが変わる（どの業種様式かはDEIの業種コードでわかる）。またIFRSにもBSの様式が2種類ある。形の全体像は次のとおり。
