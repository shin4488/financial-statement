...
XBRLの中身は、「資産合計は1,234,567,000円」のような**数値1件ずつの記録**を大量に集めたもの。この1件分の記録をXBRLの用語で**fact**と呼ぶ（以降の章でもこの名前で呼ぶ）。1つのfactは次の3つの組で構成される。
...
このほか企業が独自に定義する「企業拡張タグ」も存在するが、企業ごとに意味の保証がないため、このシステムは意図的に読まない（詳細は[08章](08_taxonomy_mapping.md)）。
...
日本基準でも、銀行・保険など一部の業種は業法に基づく独自の様式で開示する。どの業種様式かはDEIの業種コードでわかる（例: `bnk`=銀行、`INS`=保険、`cte`=一般と同等）。銀行は対応済み、保険は未対応（詳細な対応状況は[02章](02_product.md)）。
