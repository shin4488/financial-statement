...
- IFRSのBSには様式が2種類ある。流動/非流動に区分する**分類様式**と、区分せず流動性の高い順に並べる**流動性配列**（金融業に多い）。DEIでは区別できず、タグの実在で判定する

### 業種別様式

同じ日本基準でも、銀行・保険など一部の業種は業法に基づく独自の様式で開示するため、業界によって財務諸表の形そのものが変わる。どの業種様式かはDEIの業種コードでわかる。

| 業界 | 業種コード | BSの形 | PLの構造 |
|---|---|---|---|
| 一般事業会社（製造・小売・ITなど大半） | なし / `cte` | 流動/固定に区分（BS節の図） | 売上高 − 費用 → 営業利益（PL節の図） |
| 銀行 | `bnk` | 流動/固定の区分がなく、貸出金・預金などの固有科目が並ぶ（下図） | 売上高がなく、経常収益 − 経常費用 → 経常利益 |
| 保険 | `INS` | 区分がなく、保険契約準備金などの固有科目が並ぶ | 保険料等収入を含む経常収益型 |
| その他（証券など、業法に基づく様式をもつ業種がほかにも複数ある） | 上記以外のコード | 業種ごとの固有様式 | 業種ごとに異なる |
