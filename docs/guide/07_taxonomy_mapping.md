...
「実地調査の記録」より前が引くための対応表、それ以降がその根拠になった実測記録。タグを引く作業は前半だけで足り、後半は「なぜこの対応になっているか」を確かめたいときに開く。

| 知りたいこと | 節 |
|---|---|
| 形式の一覧・業種別接尾辞・名前空間・コンテキストの読み方 | 読み方 |
| BSの科目とタグ（共通科目・流動/非流動・形式固有の内訳） | BS（貸借対照表） |
| 売上・費用・利益のタグと業種によるゆれ | PL（損益計算書） |
| CFのタグとサマリ形式のタグ | CF（キャッシュ・フロー計算書） |
| `sum(...)` などの記法と、表で表せないものの扱い | 複数タグの合算とマッピング表で表せないもの |
| 企業の拡張タグしか無く科目が取れない場合 | 標準タグで取れない場合の扱い |
| タグを追加するときの手順 | 新しいタグを追加するとき |
| 上の表の根拠・検証用の実測値・業種別の全数検証 | 実地調査の記録（以降すべて） |
...
本番DBで直近1年（2025-08〜2026-08提出）に `unsupported` 判定だった436財務諸表の業種内訳は、建設 279 / 証券 26 / 海運 20 / 鉄道 18 / 電気 18 / 電気通信 18 / ガス 17 / 特定金融 9 / 保険 9 / 商品先物 5 / 複数コード 8 / 投資業・投資運用 2 / 米国基準 7 だった。業種ごとに代表企業の有報XBRLを取得（計77件）して上の対応表を作り、最後に対象261有報すべてを新実装に通して確認した（結果: 主対象235件のうち225件がBS/PL/CFすべて描画可。残りは米国基準6件、収益・費用が企業拡張タグにしか無い証券2社と投資運用1社のPL、貯金が拡張タグの日本郵政のBS）。実測に使った主な有報:
