...
**DBマイグレーション・GraphQL・フロントの変更はなし**（[04章](04_system_overview.md)の設計の効き目）。
...
[04章](04_system_overview.md)の保存行がチャートになる姿。入力（単位: 百万円）:
...
高さを揃える。銀行BSも同じ考え方で、内訳科目が非常に多いため主要科目（現金預け金・
貸出金・有価証券・預金）だけタグで取り、「その他資産」「その他負債」を合計との残差で導出する。
