...
送信イベント4種、列挙型パラメータ、UUID、数値範囲、2KiBの本文上限を検証する。任意の送信先・パラメータを受け付けない。既存nginxの `/api/` レート制限を使用し、Googleへの通信は短いタイムアウト・再試行なし。障害時は503、未設定時も503であり、成功したふりをしない。表示処理は計測の成功を待たない。
