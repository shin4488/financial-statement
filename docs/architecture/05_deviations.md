...
※ 当初の設計ドキュメント一式は削除済み（gitの履歴には残っている）。
   この文書は「なぜ設計時の想定と違う実装になったか」の判断記録として残す。
...
| 7 | sidekiq-cron を新ジョブに差し替え | **cron未登録**（ジョブ・rakeタスクのみ追加）→ 2026-08-02に差し替え済み | 旧ジョブと並走するとEDINETへのリクエストが倍増するため、切替を明示的な判断のタイミングまで遅らせた |
| 8 | `Reports::SearchQuery` | `Disclosure::SearchQuery` | モデルと同じドメイン名前空間に揃える（`Reports` と `Reporting` のような紛らわしい併存を避ける） |
| 9 | フロントは既存 `src/` を置き換え・トップページを新UI化 | `/v2` ルートに新UIを追加 → 2026-08-02にトップ昇格済み。見た目は従来と同一（MUI+カルーセル+同配色） | 新旧を見比べて切替判断できるようにするため段階を踏んだ |
