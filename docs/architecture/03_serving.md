...
バックエンドが積み上げバーの中身まで組み立てて返す。フロントは受け取った構造を描くだけで、
科目・会計基準・形式を一切知らない。
...
items_hash（入力） → PlIfrs Builder → GraphQLレスポンス（出力）:

```
入力:  pl.revenue 4,505,720 / pl.cost_of_sales 1,571,588 / pl.sga 1,084,215
       pl.profit_before_tax -142,355                          （単位: 百万円）

導出:  その他損益（純額） = pbt - (収益 - 開示済み費用)
                        = -142,355 - (4,505,720 - 2,655,803) = -1,992,272
       → 負なので費用側へ。この残差項目のおかげで借方合計 = 貸方合計が常に成立
...
出力:
  ...
  │ 売上原価: 34.8%          │  │ 収益: 100%               │
  │ 販管費: 24.0%            │  │                          │
  │ その他損益(純額): -44.2% │  │ 税引前損失: -3.1% ←赤字は│
  └────────────────┘  └───貸方に積んで高さを揃える┘
  ratioの符号: signedAmount基準（損失セグメントは負の%で届く）
```

## 実例2: 三菱UFJのBS（銀行・残差導出）

```
入力:  bs.assets 431.7兆 / bs.cash_and_equivalents 90.0兆 / bs.loans 133.8兆
       bs.securities 85.7兆 / bs.deposits 239.4兆 / bs.liabilities 408.0兆 / bs.equity 23.7兆

導出:  その他資産 = 資産合計 - (現金預け金+貸出金+有価証券) = 122.2兆
       その他負債 = 負債合計 - 預金 = 168.5兆
       （内訳科目は多数あるため、意味の大きい科目だけタグで取り残りは残差で吸収）

出力:  借方[現金預け金20.8% | 貸出金30.9% | 有価証券19.8% | その他資産28.2%]
       貸方[預金55.4% | その他負債39.0% | 純資産5.4%]
```
...
保険IFRSは収益が企業拡張タグのみで標準タグから取れない → **PLだけ**説明文になり、
BS・CFは普通に表示される。カード全体を落とさずチャート単位で劣化させる。
...
- 金額は独自スカラ `Money`（**JSON数値のまま**返す。円の最大値4e14はJSの安全整数域9e15内）
