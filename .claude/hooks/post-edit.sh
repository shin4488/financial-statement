...
for file; do
  case "$file" in
    ...
    "$project_dir"/application/backend/config.ru) backend_files+=("$file") ;;
...
output=$({
  if [ "${#backend_files[@]}" -gt 0 ]; then
    (
      ...
      bundle exec rubocop -A --force-exclusion --format emacs "${backend_files[@]}" 2>&1 |
        grep -v '\[Corrected\]'
    )
  fi
  if [ "${#frontend_files[@]}" -gt 0 ]; then
    (
      ...
      node_modules/.bin/eslint --fix "${frontend_files[@]}"
      node_modules/.bin/prettier --write "${frontend_files[@]}" >/dev/null
