    ...
    >
      ...
      <Section title="2. アクセス解析ツールについて">
        <P>
          ...
          Analyticsは、利用状況を把握するためにCookieや類似の識別子を使用してトラフィックデータを収集します。Cookie等の識別子を使って利用状況を集計します。氏名やメールアドレスは計測イベントに含めません。収集されたデータはGoogleのプライバシーポリシーに基づいて管理されます。
        </P>
        <Bullets>
          <Bullet>
            Googleによるデータの利用について:{' '}
      ...
      <Section title="ブラウザ拡張機能の利用状況について">
        <P>
          investee拡張機能は、ポップアップを開いたこと、財務データの表示結果、グラフの手動切替、リンクのクリック、拡張機能のバージョンを計測します。
          ポップアップを開いた対応サイトのドメイン名は送信しますが、閲覧URL、検索内容、証券コード、企業名、他のタブの閲覧履歴は計測に含めません。
          ブラウザ内で生成したランダムな識別子とセッション識別子を用い、当サイトのサーバを経由してGoogle
          Analyticsに送信します。利用者のIPアドレスはGoogleへの計測データに付加しません。
        </P>
        <P>
          拡張機能のポップアップ下部にある「改善のため利用状況を送信する」をOFFにすると、以後の計測を停止できます。財務データの表示は引き続き利用できます。
        </P>
      </Section>
      ...
      <Section title="4. ブラウザに保存する設定について">
        <P>
          当サイトは、グラフの自動切替のON/OFFといった表示設定を、利用者のブラウザ（ローカルストレージ）に保存します。表示の再現に使うほか、自動切替を操作した事実とON/OFFを利用状況として集計します。ブラウザのサイトデータを削除すると初期状態に戻ります。
        </P>
      </Section>
