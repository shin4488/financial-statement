...
RSpec.describe Charts::Builders::CashFlow do
  let(:items) do
    { "cf.cash_begin" => 109_095_437, "cf.operating" => -23_064_420,
      ...
      "cf.exchange_effect" => 690_400, "cf.cash_end" => 90_045_500 }
  end

  it "為替影響を含む6ステップがkind付き・符号付きで返る（単位: 百万円）" do
    chart = described_class.new(items).build
    expect(chart.renderable).to be true
    expect(chart.steps.map(&:key)).to eq %w[cashBegin operating investing financing exchangeEffect cashEnd]
    expect(chart.steps.map(&:kind)).to eq %w[balance flow flow flow flow balance]
  ...
  it "色の役割は増減の向きで決まる（負=cashDecrease。符号の解釈をフロントにさせない）" do
    ...
    expect(chart.steps.map(&:color_role))
      .to eq %w[cashIncrease cashDecrease cashIncrease cashDecrease cashIncrease cashIncrease]
  end
