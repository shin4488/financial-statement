...
- **実装済みアーキテクチャの正は `docs/guide/` の03章（データのつながり: 4層設計・取込・データモデル・チャート生成・描画）と04章（システム構成・信頼性・画面実装）**。旧系統（SecurityReport系）はコード・テーブルとも削除済み（経緯と復元手段は`docs/guide/07_unused_but_kept.md`）
