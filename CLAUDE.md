...
- **実装済みアーキテクチャの正は `docs/guide/` の深掘り章（09〜12）**
  （設計思想・データモデル・取込・表示の詳細と設計意図）。
  旧系統（SecurityReport系）のコードは削除済みで、`security_reports`テーブルのみ
  凍結保管（`docs/guide/15_legacy_cleanup.md`）
- **XBRLタグを扱う作業の前に必ず
  `docs/guide/14_xbrl_research.md`（6社実測データ）を読むこと**
...
ドキュメントの本体は `docs/guide/`（README.mdが目次）。学ぶ章（01〜08: ドメイン知識→仕様→
技術前提→全体像→実装→開発フロー→運用）+ 深掘り章（09〜12: 設計判断の記録）+
資料（13〜15: タグ対応表・実測データ・削除記録）の3部構成。
