...
- **IFRS企業の連結財務諸表は `jpigp_cor` 名前空間**（日本基準は `jppfs_cor`）。現行実装はIFRS（`ifrs_classified` / `ifrs_liquidity`、詳細タグ義務化前=2019年3月期より前の有報は経営指標サマリのみの `ifrs_summary`）と日本基準の銀行・保険にも対応済み
...
- **実装済みアーキテクチャの正は `docs/guide/` の03章（データのつながり: 4層設計・取込・データモデル・チャート生成・描画）と04章（システム構成・信頼性・画面実装）**。旧系統（SecurityReport系）のコードは削除済みで、`security_reports`テーブルのみ凍結保管（`docs/guide/08_unused_but_kept.md`）
- **XBRLタグを扱う作業の前に`docs/guide/06_taxonomy_mapping.md`のうち触る表（BS/PL/CF）の節を読むこと**（対応表だけの資料。全文を読む必要はない）。根拠の実測記録は`docs/guide/07_taxonomy_survey.md`に分離してあり、対応の理由を確かめたいときだけ開けばよい
...
ドキュメントの本体は `docs/guide/`（README.mdが目次）。学ぶ章（01〜05: ドメイン知識→仕様→財務データのつながり→システム→開発と運用。技術の基礎説明と設計判断の「なぜ」は03〜04章に織り込まれている）+ 資料（06〜08: タグ対応表・実地調査の記録・使っていないが残しているもの）の構成。
