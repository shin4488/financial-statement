...
- **実装済みアーキテクチャの正は `docs/guide/` の03章（データのつながり: 4層設計・取込・データモデル・チャート生成・描画）と04章（システム構成・信頼性・画面実装）**。旧系統（SecurityReport系）のコードは削除済みで、`security_reports`テーブルのみ凍結保管（`docs/guide/07_unused_but_kept.md`）
...
ドキュメントの本体は `docs/guide/`（README.mdが目次）。学ぶ章（01〜05: ドメイン知識→仕様→財務データのつながり→システム→開発と運用。技術の基礎説明と設計判断の「なぜ」は03〜04章に織り込まれている）+ 資料（06〜07: タグ対応表と実地調査・使っていないが残しているもの）の構成。
